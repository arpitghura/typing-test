...
startBtn.addEventListener("click", () => {
  ...
  if (dynamicTextBoxHeight < eightyPercentViewportHeight) {
    ...
    console.log(
      "Dynamic text box height is greater than or equal to 80% of viewport height."
    );
...
userInput.addEventListener("input", () => {
  ...
  if (
    timeout ||
    (input === curWord && wordIndex === extracted_words.length - 1)
  ) {
    completedSession();
  }
...
document
  .getElementById("timingSessionChoose")
  .addEventListener("click", (e) => {
    if (
      e.target.name === "time-button" &&
      (easyLvlBtn.checked || interLvlBtn.checked || hardLvlBtn.checked)
    ) {
      startBtn.style.display = "block";
      // startInstruction.style.display = 'block'
    }
...
document.getElementById("levelSelector").addEventListener("click", (e) => {
  if (
    e.target.name === "radio-button" &&
    (onemin.checked || twomin.checked || fivemin.checked)
  ) {
    startBtn.style.display = "block";
    startInstruction.style.display = "block";
  }
...
nameInput.addEventListener("keypress", (e) => {
  if (e.code === "Enter") nameSubmitBtn.click();
});
